...
size_t serialize_send_buff(char* cBuff, char* buff, size_t length) {
  ...
  size_t tmp_size = length < 1024? 1024:length;
  // fprintf(stderr, "tmp_size:%lu\tdata_size:%lu\n", tmp_size, length);
  size_t cSize = ZSTD_compress(cBuff, tmp_size, buff, length, 1);
  if (ZSTD_isError(cSize)) {
    fprintf(stderr, "error compressing : %s \n"
        "Compressed size: %ld\t before size: %lu\n",
        ZSTD_getErrorName(cSize), cSize, length);
    exit(8);
  }
  return cSize;
}
...
class Graph {
 public:
  ...
  AdjUnit<EdgeData>** incoming_adj_list;  // AdjUnit<EdgeData> [sockets]
                                          // [vertices+1]; numa-aware
  Bitmap** outgoing_adj_bitmap;
  EdgeId** outgoing_adj_index;  // EdgeId [sockets] [vertices+1]; numa-aware
  AdjUnit<EdgeData>** outgoing_adj_list;  // AdjUnit<EdgeData> [sockets]
                                          // [vertices+1]; numa-aware
...
#pragma omp parallel for
    ...
    comm_info.send_bytes = new double[partitions];
    comm_info.recv_bytes = new double[partitions];
    for (int i = 0; i < partitions; ++i) {
      comm_info.send_bytes[i] = 0;
      comm_info.recv_bytes[i] = 0;
...
// }
    ...
    for (int s_i = 0; s_i < sockets; s_i++) {
      ...
      compressed_outgoing_adj_vertices[s_i] = 0;
      for (VertexId v_i = 0; v_i < vertices; v_i++) {
        // @zc: Add a vertex and its outgoing edge index into this numa part's
        // adj index
        if (outgoing_adj_bitmap[s_i]->get_bit(v_i)) {
          outgoing_adj_index[s_i][v_i] += last_e_i;
          last_e_i = outgoing_adj_index[s_i][v_i];
          compressed_outgoing_adj_index[s_i]
                                       [compressed_outgoing_adj_vertices[s_i]]
                                           .vertex = v_i;
      ...
      // @zc: Add upper bound of vertex's outgoing index of this numa part
      for (VertexId p_v_i = 0; p_v_i < compressed_outgoing_adj_vertices[s_i];
           p_v_i++) {
        VertexId v_i = compressed_outgoing_adj_index[s_i][p_v_i].vertex;
        outgoing_adj_index[s_i][v_i] =
...
//#ifndef ZSTD
    ...
    memcpy(
        send_buffer[current_send_part_id][s_i]->data + sizeof(MsgUnit<M>) * pos,
        local_send_buffer[t_i]->data,
        sizeof(MsgUnit<M>) * local_send_buffer[t_i]->count);
//#else
    // size_t data_size = local_send_buffer[t_i]->count * sizeof(MsgUnit<M>);
    // size_t tmp_size = data_size > 1024? data_size:1024;
    // char* compressed_data = new char[tmp_size];
    // auto compressed_data_size = serialize_send_buff(compressed_data,
    //                                                 local_send_buffer[t_i]->data,
    //                                                 data_size);
    // int pos = __sync_fetch_and_add(&send_buffer[current_send_part_id][s_i]->count,
    //                                compressed_data_size);
    // memcpy(
    //     send_buffer[current_send_part_id][s_i]->data + pos,
...
//#endif
  ...
  R process_edges(
    ...
    if (sparse) {
      ...
      auto cur_part_start = partition_offset[partition_id];
      auto cur_part_end = partition_offset[partition_id + 1];
#pragma omp parallel for
      for (VertexId begin_v_i = cur_part_start; begin_v_i < cur_part_end;
...
#pragma omp parallel for
      ...
      // fprintf(stderr, "Flushed send buffer (sparse).\n");
...
#pragma omp parallel reduction(+ : reducer)
    ...
    auto comm_info_type = get_mpi_data_type<double>();
    MPI_Allreduce(MPI_IN_PLACE, comm_info.send_bytes, partitions, comm_info_type, MPI_MAX, MPI_COMM_WORLD);
    MPI_Allreduce(MPI_IN_PLACE, comm_info.recv_bytes, partitions, comm_info_type, MPI_MAX, MPI_COMM_WORLD);
